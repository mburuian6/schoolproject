 ...
 * @author Ian Mburu
 */
public class MediaSalesReport {
    Set<String> keys;
    public LinkedHashMap getData(){
        MediaSalesHandle mediaSalesHandle = BeansClass.mediaSalesHandle();
        LinkedHashMap <String,ArrayList> dateItems = mediaSalesHandle.getAllItemsPerDate();
    ...
    public DynamicReport buildReport(){
        ...
        for(int i=0; i<getNumberOfSubreports();i++){
            MediaSalesSubReport mediaSalesSubReport = BeansClass.mediaSalesSubReport();
            DynamicReport subReport;
            try {
                subReport = mediaSalesSubReport.buildReport(); 
    ...
    public static void main(String[] args) {
        MediaSalesReport test = new MediaSalesReport();
        test.produceReport();
    }
}
