 ...
 * @author Ian Mburu
 */
public class MediaInvoice {
    
    DynamicReportBuilder drb;
    JRDesignChart chart;
    public ArrayList<MediaAndCustomSaleEntry> getData(String saleId){
        AddMediaSaleLogic addMediaSaleLogic = BeansClass.addMediaSaleLogic();
        return addMediaSaleLogic.findOneSale(saleId);
    }
    
    public Date getDate(String saleId){
        ArrayList<MediaAndCustomSaleEntry> data = getData(saleId);
        if(data!=null){
            try{
            MediaAndCustomSaleEntry entry = data.get(0);
